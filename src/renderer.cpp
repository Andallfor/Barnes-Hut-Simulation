...
bool Renderer::initialize() {
    wc = {
        ...
        CS_CLASSDC,
        WndProc,
        0L, 0L, GetModuleHandle(nullptr), nullptr, nullptr, nullptr, nullptr, L"main", nullptr
    };
    ::RegisterClassExW(&wc);
    hwnd = ::CreateWindowW(
        wc.lpszClassName,
...
void Renderer::cleanupRenderTarget() {
    ...
    for (UINT i = 0; i < NUM_BACK_BUFFERS; i++) {
        if (g_mainRenderTargetResource[i]) {
            g_mainRenderTargetResource[i]->Release(); g_mainRenderTargetResource[i] = nullptr;
        }
    }
...
Renderer::FrameContext* Renderer::waitForNextFrameResources() {
...
LRESULT WINAPI WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (msg) {
    case WM_SIZE:
        if (r.g_pd3dDevice != nullptr && wParam != SIZE_MINIMIZED) {
            r.waitForFrame();
            r.cleanupRenderTarget();
            HRESULT result = r.g_pSwapChain->ResizeBuffers(0, (UINT)LOWORD(lParam), (UINT)HIWORD(lParam), DXGI_FORMAT_UNKNOWN, DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT);
